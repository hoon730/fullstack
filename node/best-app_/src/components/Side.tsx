...
const Side:React.FC<SideProps>=({setShowLogin})=>{
    ...
    return (
        <Stack gap={2} className="mx-auto w-100">
            ...
            {!authUser &&<Button variant="outline-success" as={Link as any} to="/signup">
                SignUp
            </Button>}
            {authUser &&<div className="alert alert-danger">{authUser.name}님 로그인 중 ...</div>}

            {authUser&&<Button variant="outline-success" onClick={handleLogout}>Logout</Button>}

            {!authUser&&(<Button variant="outline-success" onClick={()=>{setShowLogin(true)}}>SignIn</Button>)}
