...
app.use((req, res, next) => {
  ...
  res.write("<h1>Le gaan jaa Test!!</h1>");
  next(); // 다음 미들웨어로 넘김
});

app.use((req, res, next) => {
  console.log(`2. 미들웨어 요청 처리...`);
  req.user = "Woojin"; // user명 설정
  next();
});
...
app.get("/", (req, res) => {
  ...
  if(req.user === "Woojin") throw new Error("Woojin은 레간쟈야!!!")
  res.end(); //응답 종료
});
...
function auth(req, res, next) {
  ...
  if (!req.headers || !req.headers.authorization) {
    return res
      .status(401)
      .end(
        `<h1 style="color:yellowgreen">${req.user}님은은 인증받지 않은 사용자에요-접근불가가</h1>`
      );
  }
  next();
}
...
app.use((err, req, res, next) => {
  ...
  res.status(500).end(`<h1 style="orange">Server Error</h1>`);
});
